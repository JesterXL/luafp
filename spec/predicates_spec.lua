...
describe("src/predicates.lua", function()
    ...
    describe('isUserdata', function()
        it('should be true for c userdata', function()
            local stubType = function(o) return 'userdata' end
            assert.True(predicates.isUserdata('some c construct', stubType))
        end)
        it('should be false for number', function()
            local stubType = function(o) return false end
            assert.False(predicates.isUserdata(1, stubType))
        end)
    end)
    ...
    describe('isEmptyString', function()
        it('should be true for a blank single quote string', function()
            assert.True(predicates.isEmptyString(''))
        end)
        it('should be true for a blank double quote string', function()
            assert.True(predicates.isEmptyString(""))
        end)
        it('should be false for table', function()
            assert.False(predicates.isEmptyString({}))
        end)
    end)
    describe('isCow', function()
        it('should be true for cow', function()
            assert.True(predicates.isCow('cow'))
        end)
        it('should be true 🐮', function()
            assert.True(predicates.isCow('🐮'))
        end)
        it('should be false for 🐔', function()
            assert.False(predicates.isCow('🐔'))
        end)
    end)
